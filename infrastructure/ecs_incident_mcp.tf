...
resource "aws_ecr_lifecycle_policy" "incident_mcp_server" {
  repository = aws_ecr_repository.incident_mcp_server.name
    ...
    Statement = [
      {
        ...
        Resource = "${aws_cloudwatch_log_group.incident_mcp_server.arn}:*"
      }
    ]
  })
}
  ...
  container_definitions = jsonencode([
    {
      ...
      image     = "${aws_ecr_repository.incident_mcp_server.repository_url}:latest"
      essential = true
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.incident_mcp_server.name
...
resource "aws_ecs_service" "incident_mcp_server" {
  ...
  task_definition = aws_ecs_task_definition.incident_mcp_server.arn
  desired_count   = var.mcp_desired_count
  launch_type     = "FARGATE"

  network_configuration {
    subnets          = aws_subnet.private[*].id
    security_groups  = [aws_security_group.incident_mcp_server.id]
    assign_public_ip = false
  }

  service_registries {
    registry_arn = aws_service_discovery_service.incident_mcp_server.arn
  }
