...
ChatWindow polls `getRemediationStatus(incidentId)` every **5 seconds**, showing:
- Issue link + number
- PR link + status
- Review verdict
- Merge status

Polling pauses automatically after **30 minutes** or if status is unchanged for 5 consecutive polls.
