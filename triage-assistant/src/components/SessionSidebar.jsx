  ...
  return (
    <aside className="w-64 shrink-0 flex flex-col bg-white overflow-hidden border-r border-gray-200">
      ...
      {untriagedCount > 0 && (
        <button
          type="button"
          onClick={() => onOpenIncidents?.('cloudwatch_alarm')}
          className="mx-3 mt-2 mb-1 w-[calc(100%-1.5rem)] flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-amber-50 border border-amber-200 hover:bg-amber-100 transition-colors text-left"
        >
          {/* Pulsing red dot */}
      ...
      <div className="px-3 py-3 space-y-0.5 border-b border-gray-100">
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors ${
            untriagedCount > 0
              ? 'text-amber-700 bg-amber-50 hover:bg-amber-100 font-medium border border-amber-200'
              : 'text-gray-700 hover:bg-gray-100'
          }`}
          title={`${untriagedCount} alarm-triggered incident(s) not triaged`}
        >
          <span className="relative shrink-0">
            <svg className={`w-4 h-4 ${untriagedCount > 0 ? 'text-amber-600' : 'text-gray-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
            </svg>
            {untriagedCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-[16px] flex items-center justify-center rounded-full bg-red-500 text-white text-[9px] font-bold px-0.5">
          ...
          {untriagedCount > 0 && (
            <span className="text-[10px] font-semibold text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded-full border border-amber-200">
              {untriagedCount} new
            </span>
          )}
        </button>
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-violet-700 hover:bg-violet-50 transition-colors"
        >
